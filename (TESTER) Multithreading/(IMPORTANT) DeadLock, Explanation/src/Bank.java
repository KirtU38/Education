...
public class Bank {
    ...
    public void transfer(Account fromAccount, Account toAccount, long amount) {
        ...
        if (areSameAccount(fromAccount, toAccount) || someoneIsBlocked(fromAccount, toAccount)) {       // Если выпали одинаковые номера или какой-то из аккаунтов заблокирован
            return;
        }
        ...
        synchronized (fromAccount.compareTo(toAccount) > 0 ? fromAccount : toAccount) {
            ...
            synchronized (fromAccount.compareTo(toAccount) > 0 ? toAccount : fromAccount) {
                ...
                if (!wentThroughSecurity(fromAccount, toAccount, amount)){     // Проверка на мошенничество
                    return;
                }
