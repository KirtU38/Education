...
public class Bank {
    ...
    public void transfer(Account fromAccount, Account toAccount, long amount) {
        ...
        System.out.printf("Запрос к \"%s\" И \"%s\"   %s%n",
                fromAccount.getAccNumber(), toAccount.getAccNumber(), Thread.currentThread().getName());
        ...
        synchronized (fromAccount.compareTo(toAccount) > 0 ? fromAccount : toAccount) {
            ...
            System.out.println("from \"" + fromAccount.getAccNumber() + "\"(" + toAccount.getAccNumber() + ") прошел дальше   "
                    + Thread.currentThread().getName());

            synchronized (fromAccount.compareTo(toAccount) > 0 ? toAccount : fromAccount) {

                System.out.println("to (" + fromAccount.getAccNumber() + ")\"" + toAccount.getAccNumber() + "\" прошел дальше   "
                        + Thread.currentThread().getName());

                System.out.printf("Transfer request From: \"%s\", To: \"%s\", Amount: %d   %s%n",
                        fromAccount.getAccNumber(), toAccount.getAccNumber(), amount, Thread.currentThread().getName());
                ...
                System.out.println("+++ From \"" + fromAccount.getAccNumber() + "\" balance: " + getBalance(fromAccount) +
                        ", To \"" + toAccount.getAccNumber() + "\" Amount: " + amount + "   "
                        + Thread.currentThread().getName());
            }
        }
    }
    ...
    private boolean wentThroughSecurity(Account fromAccount, Account toAccount, long amount) {
        if (amount > 50000) {
            try {
                ...
                System.out.printf("USERS FREE: \"%s\" AND \"%s\"   %s%n", fromAccount.getAccNumber(), toAccount.getAccNumber(),
                        Thread.currentThread().getName());
    ...
    private void blockAccounts(Account fromAccount, Account toAccount) {
        ...
        System.out.printf("USERS BLOCKED: \"%s\" AND \"%s\"   %s%n", fromAccount.getAccNumber(), toAccount.getAccNumber(),
                Thread.currentThread().getName());
    }

    private boolean someoneIsBlocked(Account fromAccount, Account toAccount) {
        if (fromAccount.isBlocked() || toAccount.isBlocked()) {
            System.out.println("DENIED \"" + fromAccount.getAccNumber() + "\" OR \"" + toAccount.getAccNumber() + "\" IS BLOCKED   "
                    + Thread.currentThread().getName());
            return true;
        }
        return false;
    ...
    private boolean areSameAccount(Account fromAccount, Account toAccount) {
        if (fromAccount.getAccNumber().equals(toAccount.getAccNumber())) {
            System.out.printf("НЕЛЬЗЯ ПЕРЕВОДИТЬ НА ТОТ ЖЕ НОМЕР \"%s\"   %s%n", fromAccount.getAccNumber(),
                    Thread.currentThread().getName());
            return true;
        }
        return false;
